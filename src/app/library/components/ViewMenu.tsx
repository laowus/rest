...
const ViewMenu: React.FC<ViewMenuProps> = ({ setIsDropdownOpen }) => {
  ...
  return (
    <Menu className='view-menu no-triangle dropdown-content z-20 mt-2 border-base-100 shadow-2xl'>
      {viewOptions.map((option) => (
        <MenuItem key={option.value} label={option.label} buttonClass='h-8' toggled={viewMode === option.value} onClick={() => handleSetViewMode(option.value as LibraryViewModeType)} />
      ))}
      <hr aria-hidden='true' className='my-1 border-base-200' />
      <MenuItem label={_('Book Covers')} buttonClass='h-8' labelClass='text-sm sm:text-xs' disabled />
      {coverFitOptions.map((option) => (
        <MenuItem key={option.value} label={option.label} buttonClass='h-8' toggled={coverFit === option.value} onClick={() => handleToggleCropCovers(option.value as LibraryCoverFitType)} />
      ))}
      <hr aria-hidden='true' className='my-1 border-base-200' />
      <MenuItem label={_('Sort by...')} buttonClass='h-8' labelClass='text-sm sm:text-xs' disabled />
      {sortByOptions.map((option) => (
        <MenuItem key={option.value} label={option.label} buttonClass='h-8' toggled={sortBy === option.value} onClick={() => handleSetSortBy(option.value as LibrarySortByType)} />
      ))}
      <hr aria-hidden='true' className='my-1 border-base-200' />
      {sortingOptions.map((option) => (
        <MenuItem key={option.value.toString()} label={option.label} buttonClass='h-8' toggled={isAscending === option.value} onClick={() => handleSetSortAscending(option.value)} />
      ))}
    </Menu>
  );
};
