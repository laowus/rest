...
import { BookContent, BookConfig, PageInfo, BookProgress, ViewSettings, TimeInfo, FIXED_LAYOUT_FORMATS } from '@/types/book';
...
interface ReaderStore {
  ...
  setProgress: (key: string, location: string, tocItem: TOCItem, section: PageInfo, pageinfo: PageInfo, timeinfo: TimeInfo, range: Range) => void;
  getProgress: (key: string) => BookProgress | null;
  setView: (key: string, view: null) => void;
  getView: (key: string | null) => null | null;
  getViews: () => null[];
  getViewsById: (id: string) => null[];
  setViewSettings: (key: string, viewSettings: ViewSettings) => void;
  getViewSettings: (key: string) => ViewSettings | null;

  initViewState: (envConfig: EnvConfigType, id: string, key: string, isPrimary?: boolean, reload?: boolean) => Promise<void>;
...
export const useReaderStore = create<ReaderStore>((set, get) => ({
  ...
  initViewState: async (envConfig: EnvConfigType, id: string, key: string, isPrimary = true, reload = false) => {
  ...
  setProgress: (key: string, location: string, tocItem: TOCItem, section: PageInfo, pageinfo: PageInfo, timeinfo: TimeInfo, range: Range) =>
    set((state) => {
      const id = key.split('-')[0]!;
      const bookData = useBookDataStore.getState().booksData[id];
  ...
  getGridInsets: (key: string) => get().viewStates[key]?.gridInsets || { top: 0, right: 0, bottom: 0, left: 0 },
