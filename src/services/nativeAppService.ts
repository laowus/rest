...
const getPathResolver = ({
  ...
  const getCustomBasePrefix = getCustomBasePrefixSync ? (baseDir: BaseDir) => async () => getCustomBasePrefixSync(baseDir)() : undefined;
...
export const nativeFileSystem: FileSystem = {
  ...
  async readFile(path: string, base: BaseDir, mode: 'text' | 'binary') {
    ...
    return mode === 'text' ? (readTextFile(fp, baseDir ? { baseDir } : undefined) as Promise<string>) : ((await readFile(fp, baseDir ? { baseDir } : undefined)).buffer as ArrayBuffer);
  },
  ...
  async readDir(path: string, base: BaseDir) {
    ...
    const readDirRecursively = async (parent: string, relative: string, entries: DirEntry[], fileList: FileItem[]) => {
      for (const entry of entries) {
        if (entry.isDirectory) {
          const dir = await join(parent, entry.name);
          const relativeDir = relative ? await join(relative, entry.name) : entry.name;
          await readDirRecursively(dir, relativeDir, await readDir(dir, baseDir ? { baseDir } : undefined), fileList);
        } else {
...
export class NativeAppService extends BaseAppService {
  ...
  override hasUpdater = OS_TYPE !== 'ios' && !process.env['NEXT_PUBLIC_DISABLE_UPDATER'] && !window.__READEST_UPDATER_DISABLED;
  // orientation lock is not supported on iPad
  override hasOrientationLock = (OS_TYPE === 'ios' && getOSPlatform() === 'ios') || OS_TYPE === 'android';
  ...
  override async init() {
    ...
    if (process.env['NEXT_PUBLIC_PORTABLE_APP'] || (await this.fs.exists(`${execDir}/${SETTINGS_FILENAME}`, 'None'))) {
